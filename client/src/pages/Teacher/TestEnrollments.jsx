...
const TestEnrollments = () => {
  ...
  useEffect(() => {
    const fetchEnrollments = async () => {
      try {
        const res = await api.get(`/teacher/test/${id}/enrollments`);
          ...
          {enrollments.map((entry, idx) => (
            <li key={idx} style={{ marginBottom: '20px' }}>
              <u><strong>{entry.student.name}</strong></u> ({entry.student.email})<br />
                <ul>Roll No    : {entry.student.rollNo} | Grade: {entry.student.grade} | Institution: {entry.student.institution}<br />
                    ...
                    Payment    : {entry.paymentStatus}</ul>
